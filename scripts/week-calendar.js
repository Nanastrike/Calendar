...
import { isEventAllDay, eventEndsBefore, eventStartsBefore, initDynamicEvent,eventCollidesWith,adjustDynamicEventMaxLines } from "./event.js";
...
function initDayOfWeek(parent, selectedDate, weekDay,deviceType) {
    ...
    calendarDayOfWeekDayElement.textContent = dataFormatter.format(weekDay);
...
function calculateEventsDynamicStyles(events) {
    ...
    return initialEventGroupItems.map((eventGroupItem)=>{
        const topPercentage = 100*(eventGroupItem.startTime / 1440);
        const bottomPercentage = 100 - 100*(eventGroupItem.endTime /1440);
